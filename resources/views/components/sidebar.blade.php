    ...
    <nav class="space-y-2 flex-1 overflow-y-auto custom-scroll pr-2">
        ...
        <a href="{{ route('dashboard') }}" 
           ...
           {{ request()->routeIs('dashboard') ? 'bg-white/20 font-bold shadow-inner text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
            ...
            <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.dashboard') }}</span>
        </a>
        ...
        <a href="{{ route('rooms.index') }}" 
           ...
           {{ request()->routeIs('rooms.index') || (request()->is('rooms*') && !request()->is('rooms/maintenance*')) ? 'bg-white/20 font-bold shadow-inner text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
            ...
            <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.rooms_data') }}</span>
        </a>
        ...
        <div x-data="{ open: {{ request()->routeIs('reservations.*') || request()->is('reservasi*', 'registration*', 'check-out*') ? 'true' : 'false' }} }">
            <button type="button" 
                ...
                <div class="flex items-center space-x-3">
                    <span class="text-lg w-6 text-center">📅</span> 
                    <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.reservation') }}</span>
                </div>
            ...
            <div x-show="open" 
                ...
                <a href="{{ route('reservations.checkout.page') }}" 
                   class="block p-2 rounded-lg text-[10px] font-bold tracking-widest uppercase transition-colors {{ request()->routeIs('reservations.checkout.page') ? 'text-white bg-white/10' : 'text-white/50 hover:text-white hover:bg-white/5' }}">
                    🚪 {{ __('sidebar.check_out') }}
                </a>
                {{-- HISTORY GUEST (Rapih di dalam dropdown) --}}
        ...
        <a href="{{ route('guests.index') }}" 
           ...
           {{ request()->routeIs('guests.*') ? 'bg-white/20 font-bold shadow-inner text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
            <span class="text-lg w-6 text-center">👥</span> 
            <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.guest_book') }}</span>
        </a>
        ...
        <a href="{{ route('about.about') }}" 
           ...
           {{ request()->routeIs('about.*') ? 'bg-white/20 font-bold shadow-inner text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
            ...
            <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.about') }}</span>
        </a>
        ...
        <div class="pt-6 mt-2">
            <p class="px-3 text-[9px] uppercase text-white/40 font-black mb-3 tracking-[0.2em]">{{ __('sidebar.management') }}</p>
            ...
            <a href="{{ route('rooms.maintenance') }}" 
               ...
               {{ request()->routeIs('rooms.maintenance.*') ? 'bg-white/20 font-bold shadow-inner text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                 <span class="text-lg w-6 text-center">🛠️</span> 
                 <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.status_oo_os') }}</span>
            </a>
            ...
            <div x-data="{ open: {{ request()->routeIs('employees.*') || request()->routeIs('roles.*') ? 'true' : 'false' }} }">
                <button type="button" 
                    ...
                    <div class="flex items-center space-x-3">
                        <span class="text-lg w-6 text-center">👥</span> 
                        <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.users') }}</span>
                    </div>
                    <i class="fas fa-chevron-down text-[10px] transition-transform duration-300 opacity-70 group-hover:opacity-100" :class="open ? 'rotate-180' : ''"></i>
                </button>
            ...
            <div x-data="{ open: {{ request()->routeIs('reports.*') ? 'true' : 'false' }} }">
                <button type="button" 
                    ...
                    <div class="flex items-center space-x-3">
                        <span class="text-lg w-6 text-center">📈</span> 
                        <span class="text-[11px] font-bold tracking-[0.1em] uppercase">{{ __('sidebar.reports') }}</span>
                    </div>
                    <i class="fas fa-chevron-down text-[10px] transition-transform duration-300 opacity-70 group-hover:opacity-100" :class="open ? 'rotate-180' : ''"></i>
                </button>
       ...
       {{-- 6. SECURITY MONITORING (DROPDOWN) --}}
<div x-data="{ open: {{ request()->routeIs('admin.devices.*') || request()->routeIs('admin.ip.*') ? 'true' : 'false' }} }" class="mt-2">
    <button type="button" 
        @click="open = !open" 
        class="w-full flex items-center justify-between p-3 rounded-xl transition-all duration-200 group 
        {{ request()->routeIs('admin.devices.*') || request()->routeIs('admin.ip.*') ? 'bg-white/20 font-bold text-white shadow-inner' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
        
        <div class="flex items-center space-x-3">
            <span class="text-lg w-6 text-center">🛡️</span> 
            <span class="text-[11px] font-bold tracking-[0.1em] uppercase">Security</span>
        </div>
        <i class="fas fa-chevron-down text-[10px] transition-transform duration-300 opacity-70 group-hover:opacity-100" :class="open ? 'rotate-180' : ''"></i>
    </button>
    ...
    <div x-show="open" 
        ...
        <a href="{{ route('admin.devices.index') }}" 
           class="block p-2 rounded-lg text-[10px] font-bold tracking-widest uppercase transition-colors {{ request()->routeIs('admin.devices.index') ? 'text-white bg-white/10' : 'text-white/50 hover:text-white hover:bg-white/5' }}">
            📱 Device Activity
        </a>

        {{-- Daftar Blacklist --}}
        <a href="{{ route('admin.ip.blacklist') }}" 
           class="block p-2 rounded-lg text-[10px] font-bold tracking-widest uppercase transition-colors {{ request()->routeIs('admin.ip.blacklist') ? 'text-white bg-white/10' : 'text-white/50 hover:text-white hover:bg-white/5' }}">
            🚫 Blacklist IP
        </a>
    </div>
</div>
    ...
    <div class="pt-4 border-t border-white/20 flex-shrink-0 mt-2">
        ...
        <div class="text-[8px] text-white/30 text-center mt-4 font-bold tracking-[0.2em] uppercase">
            {{ __('sidebar.system_version') ?? 'Hotel SIG Management v2.1' }}
        </div>
    </div>
</aside>