...
<body class="bg-[#F3F4F6] antialiased h-screen flex flex-col mb-20 lg:mb-0" 
      x-data="{ openModal: false, mobileMenu: false, activeTab: 'Semua' }">
        ...
        <main class="flex-1 p-4 md:p-8 lg:p-10 overflow-y-auto custom-scroll">
            ...
            <div class="flex flex-col xl:flex-row gap-4 mb-6">
                <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                    @foreach(['Semua', 'Deluxe', 'Superior', 'Suite'] as $tab)
                ...
                <form method="GET" action="{{ route('rooms.index') }}" class="flex-1 bg-white p-2 rounded-2xl shadow-sm border border-gray-100 flex gap-2">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="room_number" value="{{ request('room_number') }}" placeholder="Cari nomor kamar..." 
                    ...
                    <button type="submit" class="bg-maroon text-white px-5 py-2 rounded-xl font-bold text-sm active:scale-95 transition-all">Cari</button>
                    @if(request('room_number'))
                    <a href="{{ route('rooms.index') }}" class="bg-gray-100 text-gray-500 px-4 py-2 rounded-xl font-bold text-sm flex items-center">Reset</a>
                    @endif
                </form>
            </div>
            ...
            {{-- Room Grid --}}
            <div class="pb-10">
                @foreach(['Semua', 'Deluxe', 'Superior', 'Suite'] as $tabType)
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-[32px] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full p-8">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-2xl font-bold text-gray-900">Tambah Kamar Baru</h3>
                ...
                <form action="{{ route('rooms.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Nomor Kamar</label>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Foto Kamar</label>
                    ...
                    <div class="mt-8 flex gap-3">
                        <button type="button" @click="openModal = false" class="flex-1 px-6 py-3 bg-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-200 transition-all">Batal</button>
                        <button type="submit" class="flex-1 px-6 py-3 bg-maroon text-white font-bold rounded-2xl hover:bg-red-900 shadow-lg shadow-red-900/20 transition-all">Simpan Kamar</button>
                    </div>
                </form>
            </div>
        </div>
