...
<body class="bg-[#F8FAFC] text-slate-700" x-data="{ 
            ...
            <main class="p-4 md:p-10 lg:p-12">
                ...
                <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/40 overflow-hidden border border-slate-100">
                    ...
                    <div class="hidden md:block overflow-x-auto table-container">
                        <table class="w-full border-collapse">
                            ...
                            <tbody class="divide-y divide-slate-50">
                                        ...
                                        <div class="flex flex-col gap-2">
                                            ...
                                            @else
                                                <div class="flex items-center gap-2 text-slate-300">
                                                    ...
                                                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5"><i class="fas fa-user-slash text-[9px]"></i> NO RESERVATION</span>
                                                </div>
                                            @endif
                                            <div class="inline-flex items-center gap-2 px-2 py-1 rounded-md w-max {{ $guest->is_incognito ? 'bg-indigo-50 text-indigo-600' : 'bg-blue-50 text-blue-600' }}">
                                                ...
                                                <span class="text-[9px] font-black uppercase tracking-tighter">{{ $guest->is_incognito ? 'Privacy On' : 'Public Profile' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-[10px] text-slate-400 flex items-center gap-2 mt-2 italic">Joined {{ $guest->created_at->diffForHumans() }}</span>
                                        </div>
                                    </td>
                                                ...
                                                <div x-show="editModal" x-transition class="bg-white rounded-[2rem] text-left overflow-hidden shadow-2xl transform transition-all w-full max-w-lg relative z-10 border border-slate-100">
                                                    <form action="{{ route('guests.update', $guest->id) }}" method="POST">
                                                        ...
                                                        <div class="bg-slate-50 px-8 py-5 flex justify-end gap-3 border-t border-slate-100">
                                                            ...
                                                            <button type="submit" class="px-6 py-3 bg-[#800000] text-white rounded-xl text-xs font-black shadow-lg shadow-[#800000]/30 hover:bg-red-900 transition-all">SIMPAN</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                                ...
                                @empty
                                <tr><td colspan="5" class="py-32 text-center text-slate-400 uppercase font-black tracking-widest">Data Kosong</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="md:hidden divide-y divide-slate-100">
                        ...
                        <div class="p-5 flex flex-col gap-4" x-data="{ editModal: false }">
                            <div class="flex items-start justify-between">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-slate-800 leading-tight">{{ $guest->is_incognito ? 'Restricted Identity' : $guest->guest_name }}</h4>
                            ...
                            <div class="grid grid-cols-1 gap-2 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                                ...
                                <div class="mt-1">
                                    <span class="text-[9px] font-black px-2 py-1 rounded {{ $guest->room && $guest->room->status == 'occupied' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700' }} uppercase">
                                        {{ $guest->room ? $guest->room->status : 'No Reservation' }}
                                    </span>
                                </div>
                            </div>
                                    ...
                                    <div x-show="editModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0" class="bg-white rounded-t-[2rem] md:rounded-[2rem] text-left overflow-hidden shadow-2xl w-full max-w-lg relative z-10 border border-slate-100">
                                        <form action="{{ route('guests.update', $guest->id) }}" method="POST">
                                            ...
                                            <div class="p-8">
                                                <div class="flex items-center gap-4 mb-6 border-b border-slate-100 pb-4">
                                                    <div class="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center text-xl"><i class="fas fa-user-edit"></i></div>
                                                    <div><h3 class="text-xl font-black text-slate-800 uppercase tracking-wider">Edit Profil</h3></div>
                                                </div>
                                                <div class="space-y-4">
                                                    <div>
                                                    ...
                                                    <div>
                                                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Telepon</label>
                                            ...
                                            <div class="bg-slate-50 px-8 py-5 flex flex-col gap-2 border-t border-slate-100">
                                                <button type="submit" class="w-full py-4 bg-[#800000] text-white rounded-xl text-xs font-black shadow-lg">SIMPAN PERUBAHAN</button>
                                                <button type="button" @click="editModal = false" class="w-full py-4 bg-white border border-slate-200 rounded-xl text-xs font-black text-slate-600">BATAL</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                        ...
                        @empty
                        <div class="py-20 text-center text-slate-400 uppercase font-black text-xs tracking-widest">Data Kosong</div>
                        @endforelse
                    </div>
    ...
    <script>
        function confirmDelete(id, name) {
            Swal.fire({
                title: 'Hapus Tamu?',
                html: `Anda yakin ingin menghapus data <b>${name}</b>?<br><span class='text-red-500 text-[10px] mt-2 block font-bold uppercase'>Data akan hilang permanen!</span>`,
                icon: 'warning',
